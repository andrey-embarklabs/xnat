...
| Any empty-target `/app` request (site root `/` → `serverRoot + "/"` → `/app`; bare `/app`; server-side redirects) → "Couldn't map Template null to any Screen class!" — **and** any ACTION whose code path ends without setting a template/redirect (e.g. `QuickSearchAction` no-match / caught-exception paths, which only `setMessage()`) → same error on POST | **`DefaultHomepageTargetValve`** (after `DetermineTargetValve`): when no explicit screen and no screen template are set — **action or not** — default the screen template to `template.homepage` (Index.vm). Plus `default.jsp` welcome file (302 root → `/app/template/Index.vm`) as belt-and-suspenders | Turbine 4.0 **dropped the `template.homepage`/`screen.homepage` auto-default**. In 2.3.3 it lived in `TemplateSessionValidator.doPerform` ("make sure we have some way to return a response"), which ran **before** the action and applied the fallback regardless of action presence; the action could still override (`TemplatePage.doBuildAfterAction` re-reads the template post-action) or redirect (`DefaultPage.doBuild` early-returns). First valve version wrongly required the action to be empty too, so message-only action paths still hit the null-template exception. Boot test guards the wiring |
...
5. Breadth: **email render path CLOSED** — exercised end-to-end against the Mailpit sink (2026-07-08):
   - `ReportIssue` action → `AdminUtils.populateVmTemplate` → `VelocityUtils.render` (`issue_report.vm` + `html_issue_report.vm`) → `MailService.sendHtmlMessage` → delivered; full context substitution, **zero unrendered `$refs`**. This validates the render-via-VelocityService rewiring (`822225ecb`-era fix) for named-template email rendering generally (`NotifyProjectListeners` uses the same `populateVmTemplate` mechanism).
   - `XDATForgotLogin` both branches (forgot-username by email; password-reset by username incl. alias-token `RESET_URL`/`RESET_LINK` substitution) → delivered, no leftover placeholder tokens.
   - Incidentally verified: the CSRF guard on anonymous Turbine actions works (tokenless POST rejected), and the anonymous login page serves a session-bound `csrfToken`.
   - NB (pre-existing upstream, verified identical on `develop`, deliberately preserved): `ReportIssue` swaps its `textBody`/`htmlBody` arguments, so the text part carries the HTML template.
